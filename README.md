...
Alternatively, if you use or commit to multiple github accounts, with multiple ssh keys or with different account names (such as having separate public and enterprise accounts), you can specify a specific username and personal access token using command line parameters, as described in the man page. Note that the default behaviour is to use the global username and personal access token without challenge.
